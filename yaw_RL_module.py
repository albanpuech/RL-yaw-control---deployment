...
def test_model(model,wind_timeseries,start_index,stop_index,ancestors,filter_duration,yaw_parameters,experiment=None,datatype='test'):
    ...
    while not done:
        ...
        action, states = model.predict(encoded, deterministic=False)
...
class Cometlogger(BaseCallback):
    ...
    Custom callback for plotting additional values in comet.
    """
